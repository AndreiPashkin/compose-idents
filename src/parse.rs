    ...
    V: Parse + Clone + Debug,
{
    fn parse(input: ParseStream) -> syn::Result<Self> {
        ...
        loop {
            ...
            match fork.parse::<Terminated<V, Token![,]>>() {
                Ok(value) => {
                    content.advance_to(&fork);
                    values.push(TupleValue::<V>::Value(value.into_value()));
                    continue;
                }
                Err(err) => {
                    errors.push(err);
...
impl Parse for LoopSourceValue {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        ...
        match input.parse::<Terminated<Expr, Token![,]>>() {
            Ok(expr) => {
                input.advance_to(&fork);
