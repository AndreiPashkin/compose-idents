...
use crate::ast::{
    Alias, AliasSpec, Ast, LoopAlias, LoopSourceValue, LoopSpec, LoopSpecItem, Scope, Spanned,
    TupleValue,
};
...
impl Resolve for LoopSpecItem {
    fn resolve<'a>(&'a self, scope: &mut Scope<'a>) -> Result<(), Error> {
        ...
        let mut names: HashMap<Alias, &'a dyn Ast> = HashMap::new();
        match self.alias() {
            LoopAlias::Simple(alias) => {
                names.insert(alias.clone(), self);
            }
